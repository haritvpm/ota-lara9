  ...
  computed: {
    configdate: function () {
      ...
      return {
        ...
        enabledDates: Object.keys(calenderdaysmap).map((x) =>
          moment(x, "DD-MM-YYYY").format("YYYY-MM-DD")
        ),
      };
    },
  ...
  methods: {
    ...
    sessionchanged: function () {
      ...
      if (this.form.duty_date != "" && this.form.duty_date != null) {
        if (
          -1 == calenderdays2[this.form.session].indexOf(this.form.duty_date)
        ) {
          this.myerrors.push(
            "For session " +
              this.form.session +
              ...
              " and " +
              calenderdays2[this.form.session][
                calenderdays2[this.form.session].length - 1
              ] +
              "."
          );
        }
      ...
      if (row.isPartime) {
        ...
        if (overtime_slot == "First") {
          if (isSittingDay) {
            ...
            if (presets_default) {
              if (
                presets_default.hasOwnProperty(
                  "default_workingday_firstot_starttime"
                ) &&
                presets_default.hasOwnProperty(
                  "default_workingday_firstot_endtime"
                )
              ) {
                def_time_start =
                  presets_default["default_workingday_firstot_starttime"];
                def_time_end =
                  presets_default["default_workingday_firstot_endtime"];
              }
            }
          } else {
            //holiday
            ...
            if (presets_default) {
              if (
                presets_default.hasOwnProperty(
                  "default_holiday_firstot_starttime"
                ) &&
                presets_default.hasOwnProperty(
                  "default_holiday_firstot_endtime"
                )
              ) {
                def_time_start =
                  presets_default["default_holiday_firstot_starttime"];
                def_time_end =
                  presets_default["default_holiday_firstot_endtime"];
              }
            }
          }
        }
        /////////SECOND////
          ...
          if (isSittingDay) {
            ...
            if (presets_default) {
              if (
                presets_default.hasOwnProperty(
                  "default_sittingday_secondot_starttime"
                ) &&
                presets_default.hasOwnProperty(
                  "default_sittingday_secondot_endtime"
                )
              ) {
                def_time_start =
                  presets_default["default_sittingday_secondot_starttime"];
                def_time_end =
                  presets_default["default_sittingday_secondot_endtime"];
              }
            }
          } else if (isWorkingDay) {
            ...
            if (presets_default) {
              if (
                presets_default.hasOwnProperty(
                  "default_workingday_secondot_starttime"
                ) &&
                presets_default.hasOwnProperty(
                  "default_workingday_secondot_endtime"
                )
              ) {
                def_time_start =
                  presets_default["default_workingday_secondot_starttime"];
                def_time_end =
                  presets_default["default_workingday_secondot_endtime"];
              }
            }
          } else if (isHoliDay) {
            //holiday
            ...
            if (presets_default) {
              if (
                presets_default.hasOwnProperty(
                  "default_holiday_secondot_starttime"
                ) &&
                presets_default.hasOwnProperty(
                  "default_holiday_secondot_endtime"
                )
              ) {
                def_time_start =
                  presets_default["default_holiday_secondot_starttime"];
                def_time_end =
                  presets_default["default_holiday_secondot_endtime"];
              }
            }
          }
        }
    ...
    insertElement: function (index) {
      ...
      this.myerrors = [];
      var prevrow =
        index > 0 && self.form.overtimes.length >= index
          ? self.form.overtimes[index - 1]
          : null;
    ...
    changeSelect(selectedOption, id) {
      ...
      self.$nextTick(() => {
        ...
        if (desig !== undefined && desig.desig) {
          ...
          row.normal_office_hours =
            desig.desig_normal_office_hours;
          ...
          console.log(row)
          if( 0 == id){
          const { def_time_start,def_time_end  } = 
            this.getDefaultTimes(this.form.overtime_slot, row) 
        ...
        if (desig !== undefined && self.form.overtimes[id].punching) {
          axios
            .get(
              urlajaxgetpunchtimes +
                "/" +
                self.form.duty_date +
                "/" +
                self.form.overtimes[id].pen
            )
            .then((response) => {
              ...
              if (
                response.data &&
                response.data.hasOwnProperty("punchin") &&
                response.data.hasOwnProperty("punchout")
              ) {
    ...
    checkDuplicates() {
      ...
      for (var i = 0; i < self.form.overtimes.length; i++) {
        if (obj[self.form.overtimes[i].pen] == undefined) {
          ...
          this.myerrors.push(
            "Duplicate name found: " + self.form.overtimes[i].pen
          );
          return false;
        }
      }
    ...
    checkTimeIsAsPerGOOnSittingDay(overtime_slot, row) {
      if (row.isPartime) {
        ...
        if (overtime_slot == "First") {
          if (row.from != "06:00" || row.to != "11:30") {
            this.myerrors.push(
              "Row " +
                (i + 1) +
                ": Parttime employee - time shall be 06:00 to 11:30 as per G.O on a sitting day"
            );
            return false;
          }
        } else if (overtime_slot == "Second") {
          ...
          if (row.from != "14:00" || row.to != "16:30") {
            this.myerrors.push(
              "Row " +
                (i + 1) +
                ": Parttime employee - time shall be 14:00 to 16:30 as per G.O on a sitting day"
            );
            return false;
          }
        } else {
        ...
        if (overtime_slot == "First") {
          if (row.from != "06:00" /*|| row.to != "16:30"*/) {
            this.myerrors.push(
              "Row " +
                (i + 1) +
                ": Fulltime employee - time shall be from 06:00 a.m. as per G.O on a sitting day"
            );
            return false;
          }
        }
        ...
        if (
          (overtime_slot == "First" &&
            (row.from != "08:00" || row.to != "17:30")) ||
          (overtime_slot == "Second" &&
            row.from != "17:30" /*|| row.to != "20:00" */) ||
          (overtime_slot == "Third" && row.from != "20:00")
        ) {
          this.myerrors.push(
            "Row " + (i + 1) + ": Time should be as per G.O on a sitting day"
          );
          return false;
        }
      }
    ...
    checkIfOTOverlapsWithOfficeHours(
      overtime_slot,
      datefrom,
      dateto,
      sNormalStart,
      ...
      var isoverlap =
        (time730am < dateto && time530pm > datefrom) ||
        time730am == datefrom ||
        time530pm == dateto;
    ...
    const isSittingOrWorkingDay =
      calenderdaysmap[this.form.duty_date].indexOf("oliday") == -1;
    const isWorkingDay =
      calenderdaysmap[this.form.duty_date].indexOf("orking") != -1;
    const isHoliDay =
      calenderdaysmap[this.form.duty_date].indexOf("oliday") != -1;
    ...
    rowsvalid() {
      ...
      if (-1 == calenderdays2[self.form.session].indexOf(self.form.duty_date)) {
        this.$swal(
          "Error",
          "The duty date is not a calender date for the session: " +
            self.form.session,
          "error"
        );
        return false;
      ...
      for (var i = 0; i < self.form.overtimes.length; i++) {
        ...
        if (
          ...
          this.$swal(
            "Row: " + (i + 1),
            "Fill all the fields in every row",
            "error"
          );
          return false;
        }
        ...
        if (self.form.overtimes[i].punching) {
          if (
            ...
            this.$swal(
              "Row: " + (i + 1),
              "Fill punch in/out time for every row",
              "error"
            );
            return false;
          }
      ...
      if (self.form.overtime_slot == "Additional") {
        if (
          ...
          this.$swal(
            "Error",
            "Only DS or above can have Additional OT!",
            "error"
          );
          return false;
        }
      ...
      for (var i = 0; i < self.form.overtimes.length; i++) {
        ...
        if (
          row.from.toLowerCase() == "invalid date" ||
          row.to.toLowerCase() == "invalid date"
        ) {
          row.from = row.to = "";
          this.myerrors.push(
            "Row " +
              (i + 1) +
              ": Invalid time format. Enter (HH:MM) in 24 hour format ( examples: 09:30, 17:30)."
          );
          return false;
        }
        ...
        if (!this.checkTimeWithinPunchingTime(row)) {
          this.myerrors.push(
            "Row " + (i + 1) + ": OT period should be within punching times"
          );
          return false;
        }
        ...
        if (isSittingOrWorkingDay) {
          ...
          if (overtime_slot == "First" && diffhours < othours_needed) {
            this.myerrors.push(
              `Row  ${
                i + 1
              } : At least ${othours_needed} hours needed for First OT on a ${daytypedesc}`
            );
            return false;
          }
        }
          ...
          if (isSittingDay) {
            if (overtime_slot === "Third") {
              this.myerrors.push(
                "Row " +
                  (i + 1) +
                  ": Parttime employees cannot have third OT on sitting day"
              );
              return false;
            }
          } else if (isWorkingDay) {
            if (overtime_slot !== "First") {
              this.myerrors.push(
                "Row " +
                  (i + 1) +
                  ": Parttime employees cannot have second/third OT on working day"
              );
              return false;
            }
          }
          ...
          if (isSittingOrWorkingDay) {
            if (overtime_slot !== "First") {
              this.myerrors.push(
                "Row " +
                  (i + 1) +
                  ": FullTime employees cannot have second/third OT on working/sitting day"
              );
              return false;
            }
          }
        ...
        if (
          ...
          if (isSittingDay || isWorkingDay) {
            if (overtime_slot !== "First") {
              ...
              if (
                !this.checkIfOTOverlapsWithOfficeHours(
                  overtime_slot,
                  datefrom,
                  dateto,
                  sNormalStart,
                ...
                this.myerrors.push(
                  `Row ${
                    i + 1
                  } : OT cannot be between ${sNormalStart} and ${sNormalEnd} am on a ${daytypedesc}`
                );
                return false;
              }
            }
        ...
        .then((response) => {
          ...
          if (response.data.created) {
            window.location.href =
              urlformsucessredirect + "/" + response.data.id;
          } else {
