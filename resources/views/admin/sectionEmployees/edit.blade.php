    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.section-employees.update", [$sectionEmployee->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('section_or_offfice') ? 'is-invalid' : '' }}" name="section_or_offfice_id" id="section_or_offfice_id" required>
                    @foreach($section_or_offfices as $id => $entry)
                        <option value="{{ $id }}" {{ (old('section_or_offfice_id') ? old('section_or_offfice_id') : $sectionEmployee->section_or_offfice->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_from') ? 'is-invalid' : '' }}" type="text" name="date_from" id="date_from" value="{{ old('date_from', $sectionEmployee->date_from) }}">
